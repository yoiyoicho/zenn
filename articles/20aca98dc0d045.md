---
title: "Ruby on RailsでOpenAI APIを使おう【Faraday / エラーハンドリング】"
emoji: "👋"
...
昨今はAIといえばPythonという風潮ですが、その流れに反してRuby on RailsアプリにChatGPTを導入してみました。
...
というのも、実際にOpenAI APIを叩いてみると、500エラーがまあまあの頻度で起こったり、リクエストが複雑化するとレスポンスにものすごく時間がかかったり、日本語でリクエストを送ったのに文字化けしたレスポンスが帰ってくるなど、ChatGPTをコンソールで使用するときと同じような動作の不安定さが気になります。

よってOpenAI APIを利用する際は適切にエラーハンドリングできる環境が大事だと感じ、ミドルウェアを使ってロギングやタイムアウトなどの追加機能を簡単に統合できる`Faraday`が適しているのではないかと結論づけた次第です。
...
まず、Railsに`faraday`をインストールします。環境変数の管理に`dotenv-rails`を使うので一緒にインストールします。
...
これで何らかの不具合が起きてOpenAI APIからのレスポンスに時間がかかっているとき、無駄にリソースを消費することを防げます。
...
`Faraday`にはエラーステータスコードが発生したときに例外を起こす`raise_error`というミドルウェアもあるので、こちらを使ったパターンも検討の価値ありです。
...
APIの設計において、リソースの変更が発生しないのにPOSTリクエストにした理由は、リクエストボディに含められるデータ量に制限がないことと、このリクエストにサーバーに対して何らかの処理を行わせる意図があることを考慮してPOSTリクエストにしました。
...
Openaiサービスクラスが発生させたエラー例外は、コントローラ内で捕捉し、500のステータスコードとともにエラーメッセージのレスポンスに変換しています。
...
返事がきました！　自分で設計したChatGPT botなら、かわいらしさもひとしおです。
