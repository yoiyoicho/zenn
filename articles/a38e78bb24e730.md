...
終了後の解説では、「現実世界でも度々問題になっている、S3バケットの設定不備によって情報が漏洩してしまうケースをモチーフにした問題」と説明があり、確かに内部にあるはずのリソースにこんなに簡単にアクセスできてしまうのかと実感できました。
...
GPTがあればサイバー攻撃だってわけもわからずできてしまうのかの怖くなります。

作問者の方のブログを読むと、SSRF攻撃を行う問題だったみたいです。

https://scgajge12.hatenablog.com/entry/security_jaws_days_2023

SSRF、徳丸本に出てきなあ。復習しなきゃ！

## まとめ

Security-JAWSのCTFに参加することで、AWSの各種サービスの知識やAWS CLIの使い方、IAMポリシーの重要性などをハンズオンで学ぶことができました。
